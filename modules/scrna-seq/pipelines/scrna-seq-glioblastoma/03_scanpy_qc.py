...
Pipeline / 파이프라인:
    ...
    -> draft brain-tumor cell-type annotation from a marker dictionary.
...
Outputs (to ~/ghbio-tutorial/results/):
    ...
    celltype_draft.csv       cluster -> draft brain-tumor cell type (from markers)
...
# -----------------------------------------------------------------------------
# Canonical glioblastoma / brain-tumor marker dictionary
# 교모세포종(뇌종양) 조직의 대표적인 세포 유형 마커 유전자 사전.
# Used to turn cluster markers into a draft cell-type label.
#
# Unlike blood (PBMC), a dissociated glioblastoma contains malignant glial cells
# plus the surrounding tumor microenvironment: normal glia, neurons, immune
# infiltrate, and vasculature. These panels are widely used in glioma scRNA-seq.
# -----------------------------------------------------------------------------
BRAIN_TUMOR_MARKERS = {
...
def annotate_celltypes(top_genes_per_cluster):
    """Score each cluster against the brain-tumor marker dictionary -> draft label.
    각 클러스터의 상위 마커를 뇌종양 마커 사전과 대조해 세포 유형 초안을 부여합니다.
    ...
    for cluster, top_genes in top_genes_per_cluster.items():
        ...
        scores = {}
        for celltype, markers in BRAIN_TUMOR_MARKERS.items():
