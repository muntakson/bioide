...
Pipeline / 파이프라인:
    ...
    -> draft T-cell subset annotation from a marker dictionary.
...
Outputs (to ~/ghbio-tutorial/results/):
    ...
    celltype_draft.csv       cluster -> draft T-cell subset (from markers)
...
# -----------------------------------------------------------------------------
# T-cell / lymphocyte marker dictionary
# 사람 T세포(림프구) 하위 유형 마커 유전자 사전.
# Used to turn cluster markers into a draft cell-type label.
#
# This dataset is FACS-sorted human pan T cells, so clusters are mostly T-cell
# SUBSETS (CD4/CD8, naive vs memory/effector, Treg, proliferating) with a little
# NK, and occasional contaminating B cells / monocytes. These panels are standard
# in human PBMC / T-cell scRNA-seq.
# -----------------------------------------------------------------------------
T_CELL_MARKERS = {
    "T cell (pan)":        ["CD3D", "CD3E", "CD3G", "CD2", "TRAC"],
    "CD4+ T":              ["CD4", "IL7R", "CD40LG"],
    "CD8+ T":              ["CD8A", "CD8B", "GZMK"],
    "Naive T":             ["CCR7", "SELL", "TCF7", "LEF1"],
    "Memory/Effector T":   ["GZMB", "GNLY", "NKG7", "CCL5", "PRF1"],
    "Regulatory T (Treg)": ["FOXP3", "IL2RA", "CTLA4", "IKZF2"],
    "Proliferating T":     ["MKI67", "TOP2A"],
    "NK cell":             ["NCAM1", "KLRD1", "NKG7", "GNLY"],
    "B cell (contam.)":    ["CD79A", "MS4A1", "CD19"],
    "Monocyte (contam.)":  ["LYZ", "S100A8", "CD14"],
}
...
def annotate_celltypes(top_genes_per_cluster):
    """Score each cluster against the T-cell marker dictionary -> draft label.
    각 클러스터의 상위 마커를 T세포 마커 사전과 대조해 세포 유형 초안을 부여합니다.
    ...
    for cluster, top_genes in top_genes_per_cluster.items():
        ...
        scores = {}
        for celltype, markers in T_CELL_MARKERS.items():
