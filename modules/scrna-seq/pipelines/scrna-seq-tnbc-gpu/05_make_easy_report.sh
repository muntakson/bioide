...
# =============================================================================
# 05_make_easy_report.sh  (TNBC 삼중음성 유방암 scRNA-seq · GPU 재현)
# Turn the high-school-level report markdown (written by the AI panel's
# "🎓 고등학생 버전 보고서" button) into ONE branded GHBIO PDF.
#
# Self-contained: it needs ONLY the report markdown. The QC UMAP figure is
# folded in when present, but nothing here depends on the other figures.
#
# Filename contract: the AI panel opens a FIXED path, so this MUST emit exactly:
#   <results>/GHBIO_고등학생_리포트.pdf
#
# Requires: pandoc, wkhtmltopdf.
# =============================================================================

RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/scrna-seq-tnbc-gpu/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성"
...
# --- Assemble the report markdown (report body + optional figures) ------------
REPORT_MD="$TMP/report.md"
cat "$SRC" > "$REPORT_MD"
if [[ -f "${RESULTS}/umap_tumor_normal.png" ]]; then
  {
    printf '\n\n## 종양세포 지도 (UMAP)\n\n'
    printf '![CNV로 판정한 종양세포(빨강)와 정상세포(파랑)](%s)\n' "${RESULTS}/umap_tumor_normal.png"
    printf '\n*그림: 점 하나가 세포 하나입니다. 유전체가 뒤틀린(이배수성) 암세포를 빨강, 정상세포를 파랑으로 칠했어요.*\n'
  } >> "$REPORT_MD"
elif [[ -f "${RESULTS}/umap_clusters.png" ]]; then
  {
    printf '\n\n## 분석 지도 (UMAP)\n\n'
...
printf '\n\n<div class="foot">GHBIO Co-Scientist · TNBC 삼중음성 유방암 scRNA-seq (GPU 재현) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"
