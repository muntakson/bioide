...
SAMPLE="Pu et al., Nature Communications 2021 — 갑상선유두암(PTC) 단일세포 (158,577 cells · 11 patients · 정상곁·원발·림프절/원격 전이)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_ptc_pu2021_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building PTC independent-reanalysis report PDF"
...
cover = f"""<div class="cover">
  ...
  <div class="sub">갑상선유두암(PTC) <b>독립 GPU 재현</b> 리포트 — Pu 2021</div>
  <div class="rule"></div>
  <div class="meta">
...
FIGS = [
  ...
  ("umap_malignant.png","독립 악성/정상 갑상선세포 판정 (비지도 GMM)",
   "갑상선(여포)세포를 PTC 악성 서명 점수로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("composition.png","조직별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, 정상곁·원발종양·전이 조직의 조성 비교(%)."),
]
