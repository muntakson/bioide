...
# =============================================================================
# 05_make_report.sh  (Choi 2023 head&neck — INDEPENDENT GPU reanalysis report)
...
SAMPLE="Choi et al., Nature Communications 2023 — 두경부암(head & neck cancer) 진행 단계 단일세포 (54,239 cells · 23 patients · 37 samples · NL/LP/CA/LN)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_hnscc_choi2023_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building head&neck independent-reanalysis report PDF"
...
cells=meta.get("cells","54239"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
...
cover = f"""<div class="cover">
  ...
  <div class="sub">두경부암(head &amp; neck cancer) <b>독립 GPU 재현</b> 리포트 — Choi 2023</div>
  <div class="rule"></div>
  <div class="meta">
    ...
    방법(헌장 제1조): 저자 라벨 미사용 · 저자 UMI 행렬 → GPU(PyTorch) 스케일·PCA → Harmony → Leiden → marker 주석 → 비지도 GMM 악성 분리 → 저자 라벨·진행 주장 대비 독립 검증
  </div>
  <div class="fields">
...
# --- validation section (verdict + bars + summary table) ----------------------
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Choi 2023)의 '
       '<b>저자 라벨</b>과 <b>진행 단계 주장</b>(백반증부터 악성세포 출현)과 대조했습니다. 아래는 두 분석이 '
...
for fn, cap in [("validation_bars.png","일치도 지표 (세포유형 ARI·NMI·악성 판정 F1)"),
                ...
                ("progression_malignant.png","진행 단계별 악성 상피세포 비율 — 논문 핵심 주장")]:
    if exists(fn):
...
FIGS = [
  ...
  ("umap_stage.png","진행 단계 UMAP (NL→LP→CA→LN)",
   "각 세포가 어느 단계(정상·백반증·암·림프절전이) 시료에서 왔는지를 색으로 표시했습니다. 단계는 세포 라벨이 아니라 시료 정보입니다."),
  ("umap_malignant.png","독립 악성/정상 상피세포 판정 (비지도 GMM)",
   "상피세포를 편평상피암 악성 서명 − 정상 편평 분화점수로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("composition.png","진행 단계별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, NL·LP·CA·LN 단계의 조성 비교(%)."),
]
