...
# =============================================================================
# 05_make_easy_report.sh  (두경부암 head&neck scRNA-seq — 고등학생 버전 보고서)
# Turn the high-school-level report markdown (written by the AI panel's
# "🎓 고등학생 버전 보고서" button) into ONE branded GHBIO PDF.
#
# Self-contained: it needs ONLY the report markdown. The UMAP figure is folded
# in when present, but nothing here depends on the other Scanpy pngs.
#
...
RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/hnscc-progression-choi2023/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (두경부암 head&neck)"
...
if [[ -f "${RESULTS}/progression_malignant.png" ]]; then
  {
    printf '\n\n## 진행 단계별 암세포 비율\n\n'
    printf '![정상→백반증→암→림프절전이로 갈수록 암세포 비율이 어떻게 변하는지](%s)\n' "${RESULTS}/progression_malignant.png"
    printf '\n*그림: 정상(NL)에서 백반증(LP), 암(CA), 전이(LN)로 갈수록 암세포(악성 상피세포) 비율이 어떻게 달라지는지 보여줍니다.*\n'
  } >> "$REPORT_MD"
fi
printf '\n\n<div class="foot">GHBIO Co-Scientist · 두경부암(head &amp; neck cancer) 진행 단계 단일세포 scRNA-seq (Choi 2023) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

# --- Render --------------------------------------------------------------------
pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 두경부암 단일세포" \
