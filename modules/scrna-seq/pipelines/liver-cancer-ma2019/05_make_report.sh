...
# =============================================================================
# 05_make_report.sh  (Ma 2019 liver — INDEPENDENT GPU reanalysis report)
...
SAMPLE="Ma et al., Cancer Cell 2019 — 간암(liver cancer) 단일세포 (~9,900 cells · 19 patients · HCC + iCCA)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_liver_ma2019_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building liver independent-reanalysis report PDF"
...
cells=meta.get("cells","9900"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
n_mal=meta.get("malignant_epithelial_cells","?"); n_hpc=meta.get("hpc_like_epithelial_cells","?")

logo = b64(os.environ["LOGO"])
cover = f"""<div class="cover">
  <img class="logo" src="data:image/svg+xml;base64,{logo}">
  <h1>GHBIO AI Co-Scientist</h1>
  <div class="sub">간암(liver cancer) <b>독립 GPU 재현</b> 리포트 — Ma 2019</div>
  <div class="rule"></div>
  <div class="meta">
    데이터셋: {html.escape(os.environ['SAMPLE'])}<br>
    <b>{cells} cells</b> · 우리 세포유형 <b>{ntypes}</b> · Leiden 클러스터 <b>{nclust}</b> · 악성/HPC-like 상피세포 <b>{n_mal}</b>/<b>{n_hpc}</b><br>
    방법(헌장 제1조): 저자 라벨 미사용 · 저자 10x 카운트 행렬 → GPU(PyTorch) 스케일·PCA → Harmony → Leiden → marker 주석 → 비지도 GMM 악성 분리 → 저자 라벨 대비 독립 검증
  </div>
  <div class="fields">
...
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Ma 2019)의 '
       '<b>공개 세포유형 라벨</b>(Type)과 대조했습니다. 아래는 두 분석이 얼마나 수렴하는지에 대한 '
       '정량 지표(ARI/NMI·혼동행렬·악성 판정 정확도)와 판정입니다.</p>']
...
for fn, cap in [("validation_bars.png","일치도 지표 (ARI/NMI·세포유형 정확도·악성 판정)"),
                ("confusion_celltype.png","세포유형 혼동행렬 — 저자 라벨이 우리 어느 유형으로 갔는지")]:
    if exists(fn):
...
FIGS = [
  ...
  ("umap_malignant.png","독립 악성/HPC-like 상피세포 판정 (비지도 GMM)",
   "상피세포를 HCC/iCCA 악성 서명 − 간 전구세포(HPC) 서명으로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("composition.png","배치(Set)별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, 두 10x 배치(Set1/Set2)의 조성 비교(%)."),
]
...
if os.path.exists(os.path.join(R,"malignant_epithelial.csv")):
    ...
    appendix+=("<h2>부록 B. 악성 vs HPC-like 상피세포 DE (우리 판정 기준)</h2>"
...
print(f"   cover: {cells} cells, {ntypes} cell types, {nclust} clusters, malignant {n_mal}/HPC-like {n_hpc}")
PY
