...
# =============================================================================
# 05_make_easy_report.sh  (간암 liver scRNA-seq — 고등학생 버전 보고서)
...
RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/liver-cancer-ma2019/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (간암 liver)"
...
printf '\n\n<div class="foot">GHBIO Co-Scientist · 간암(liver cancer) 단일세포 scRNA-seq (Ma 2019) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

# --- Render --------------------------------------------------------------------
pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 간암 단일세포" \
