...
Pipeline / 파이프라인:
    ...
    -> draft NSCLC cell-type annotation from a marker dictionary.
...
Outputs (to ~/ghbio-tutorial/results/):
    ...
    celltype_draft.csv       cluster -> draft NSCLC cell type (from markers)
...
# -----------------------------------------------------------------------------
# NSCLC (non-small cell lung cancer) tumor microenvironment marker dictionary
# 사람 폐암(NSCLC) 조직의 대표적인 세포 유형 마커 유전자 사전.
# Used to turn cluster markers into a draft cell-type label.
#
# A dissociated lung tumor contains malignant/epithelial cells plus the tumor
# microenvironment: immune cells (T/NK/B/myeloid/mast), endothelium, and
# fibroblasts. These panels are standard in lung-cancer scRNA-seq.
# -----------------------------------------------------------------------------
LUNG_TUMOR_MARKERS = {
    "Epithelial/Tumor":   ["EPCAM", "KRT19", "KRT18", "KRT8"],
    "Alveolar type II":   ["SFTPC", "SFTPB", "SFTPA1", "NAPSA"],
    "Alveolar type I":    ["AGER", "PDPN", "CAV1"],
    "Club/Secretory":     ["SCGB1A1", "SCGB3A1"],
    "Ciliated":           ["FOXJ1", "TPPP3", "PIFO"],
    "T cell":             ["CD3D", "CD3E", "CD2", "CD8A", "IL7R"],
    "NK cell":            ["NKG7", "GNLY", "KLRD1"],
    "B / Plasma":         ["CD79A", "MS4A1", "MZB1", "IGHG1"],
    "Myeloid/Macrophage": ["LYZ", "CD68", "CD14", "MARCO", "FCGR3A"],
...
def annotate_celltypes(top_genes_per_cluster):
    """Score each cluster against the NSCLC marker dictionary -> draft label.
    각 클러스터의 상위 마커를 폐암(NSCLC) 마커 사전과 대조해 세포 유형 초안을 부여합니다.
    ...
    for cluster, top_genes in top_genes_per_cluster.items():
        ...
        scores = {}
        for celltype, markers in LUNG_TUMOR_MARKERS.items():
...
# Lineage figures à la Lambrechts et al., Nat Med 2018 (Figs 2-5): for each lineage, a UMAP with its
# clusters highlighted + a per-cluster z-scored heatmap of that lineage's marker/subtype gene panel.
LINEAGE_FIGS = [
    {"out": "nsclc_endothelial.webp", "label": "Endothelial",
     "title": "Endothelial cell clusters", "cite": "Lambrechts et al., Nat Med 2018 (Fig. 2)",
     "highlight": ["Endothelial"],
     "panel": [("EC identity", ["PECAM1", "VWF", "CLDN5", "FLT1"]),
               ("Lymphatic", ["PDPN", "PROX1"]),
               ("Angiogenesis (tumor-EC UP)", ["HSPG2", "ANGPT2", "HIF1A", "MMP2"]),
               ("Immune homing (tumor-EC DOWN)", ["ICAM1", "CCL2", "HLA-DRB1"])]},
    {"out": "nsclc_fibroblast.webp", "label": "Fibroblast",
     "title": "Fibroblast clusters", "cite": "Lambrechts et al., Nat Med 2018 (Fig. 3)",
     "highlight": ["Fibroblast"],
     "panel": [("Fibroblast identity", ["COL1A1", "DCN", "LUM", "PDGFRB"]),
               ("Collagen sets", ["COL3A1", "COL4A1", "COL6A2", "COL10A1"]),
               ("Myofibroblast/Pericyte", ["ACTA2", "MYH11", "RGS5"]),
               ("Subtype", ["MMP3", "FIGF", "PLA2G2A"])]},
    {"out": "nsclc_tcell.webp", "label": "T cell",
     "title": "T-cell clusters", "cite": "Lambrechts et al., Nat Med 2018 (Fig. 5)",
     "highlight": ["T cell"],
     "panel": [("T identity", ["CD3D", "CD3E", "CD2"]),
               ("CD4 / CD8", ["CD4", "IL7R", "CD8A", "CD8B"]),
               ("Treg", ["FOXP3", "CTLA4", "IL2RA"]),
               ("Cytotoxic", ["GZMB", "GNLY", "NKG7", "PRF1"]),
               ("Checkpoint", ["PDCD1", "LAG3", "TIGIT", "HAVCR2"]),
               ("Proliferating", ["MKI67"])]},
    {"out": "nsclc_bcell_myeloid.webp", "label": "B / Myeloid / Mast",
     "title": "B-cell and myeloid-like cell clusters", "cite": "Lambrechts et al., Nat Med 2018 (Fig. 4)",
     "highlight": ["B / Plasma", "Myeloid/Macrophage", "Mast cell"],
     "panel": [("B / Plasma", ["MS4A1", "CD79A", "MZB1", "IGHG1", "JCHAIN"]),
               ("Myeloid", ["LYZ", "CD68", "CD14"]),
               ("Macrophage M1/M2", ["CD163", "MARCO", "FCGR3A"]),
               ("DC / Mast", ["CLEC9A", "FCER1A", "TPSAB1", "CPA3"])]},
]


def plot_lineage(adata, celltype_df, cfg):
    """One lineage figure: UMAP with the lineage's clusters highlighted + per-cluster z-scored
    heatmap of its marker panel. Uses adata.raw (log-normalized, all genes)."""
    hl_clusters = set(
        celltype_df.loc[celltype_df["draft_celltype"].isin(set(cfg["highlight"])), "cluster"].astype(str)
    )
    raw = adata.raw if adata.raw is not None else adata
    genes = [g for _, gs in cfg["panel"] for g in gs if g in set(raw.var_names)]
    if not genes:
        print(f"==> (skip {cfg['out']}: panel genes not in matrix)")
        return
    fig = plt.figure(figsize=(12, 5))
    # Panel A - UMAP with highlighted clusters.
    ax1 = fig.add_subplot(1, 2, 1)
    um = adata.obsm["X_umap"]
    is_hl = adata.obs["leiden"].astype(str).isin(hl_clusters).values
    ax1.scatter(um[~is_hl, 0], um[~is_hl, 1], s=3, c="#d5dbe0", linewidths=0)
    if is_hl.any():
        ax1.scatter(um[is_hl, 0], um[is_hl, 1], s=6, c="#c0392b", linewidths=0,
                    label=f"{cfg['label']} ({int(is_hl.sum())} cells)")
        ax1.legend(loc="best", fontsize=8, markerscale=2)
    ax1.set_title(cfg["title"]); ax1.set_xticks([]); ax1.set_yticks([])
    ax1.set_xlabel("UMAP1"); ax1.set_ylabel("UMAP2")
    # Panel B - per-cluster mean expression (z per gene).
    ax2 = fig.add_subplot(1, 2, 2)
    X = raw[:, genes].X
    X = X.toarray() if hasattr(X, "toarray") else np.asarray(X)
    dfe = pd.DataFrame(X, columns=genes); dfe["leiden"] = adata.obs["leiden"].values
    m = dfe.groupby("leiden")[genes].mean()
    z = (m - m.mean()) / m.std().replace(0, 1)
    im = ax2.imshow(z.T.values, aspect="auto", cmap="RdBu_r", vmin=-2, vmax=2)
    ax2.set_yticks(range(len(genes))); ax2.set_yticklabels(genes, fontsize=6.5)
    ax2.set_xticks(range(len(m.index)))
    ax2.set_xticklabels([f"{c}*" if c in hl_clusters else str(c) for c in m.index], fontsize=7)
    ax2.set_xlabel("Leiden cluster  (* = highlighted lineage)")
    ax2.set_title("Marker / subtype expression (z-score)")
    fig.colorbar(im, ax=ax2, shrink=0.6, label="z-score")
    fig.suptitle(f"{cfg['title']} - {cfg['cite']} style", fontsize=11)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    out = os.path.join(RESULTS_DIR, cfg["out"])
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"==> Saved lineage figure: {out}")


def plot_stromal_distribution(summary):
    """Stromal (non-tumor) cell-type distribution — the composition underlying the paper's stromal
    survival-marker analysis. NOTE: the survival association itself is derived in Lambrechts 2018 from
    bulk RNA-seq + clinical outcome of 1,572 patients (TCGA) and is NOT computable from one scRNA
    sample; this panel shows only the distribution.
    간질(비종양) 세포 분포. 생존 연관성은 논문의 TCGA 분석 결과이며 단일 샘플에서는 계산할 수 없습니다."""
    by_type = summary.groupby("draft_celltype")["n_cells"].sum()
    stromal = by_type.drop(labels=[t for t in ["Epithelial/Tumor"] if t in by_type.index], errors="ignore")
    stromal = stromal[stromal > 0]
    if stromal.empty:
        print("==> (skip stromal distribution: no stromal cells)")
        return
    total = int(stromal.sum()) or 1
    labels = list(stromal.index)[::-1]; vals = [int(v) for v in stromal.values][::-1]
    colors = plt.cm.Set2(np.linspace(0, 1, max(len(labels), 1)))
    fig, ax = plt.subplots(figsize=(8, max(3.0, 0.55 * len(labels) + 1.4)))
    bars = ax.barh(labels, vals, color=colors)
    for b, v in zip(bars, vals):
        ax.text(v, b.get_y() + b.get_height() / 2, f" {v} ({100.0 * v / total:.1f}%)",
                va="center", ha="left", fontsize=9)
    ax.set_xlabel("Number of stromal (non-tumor) cells"); ax.margins(x=0.18)
    ax.set_title("Distribution of stromal cells in the tumor sample "
                 "(survival-marker link per Lambrechts et al., Nat Med 2018 - TCGA, not computed here)")
    fig.tight_layout()
    out = os.path.join(RESULTS_DIR, "nsclc_stromal_distribution.webp")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"==> Saved stromal distribution: {out}")

def plot_composition(summary):
    """Bar chart of cells per draft NSCLC cell type — the tumor-microenvironment 'catalog' that is
    the central result of Lambrechts et al., Nat Med 2018 (Fig. 1). Draft labels come from the
    LUNG_TUMOR_MARKERS panel; cells are grouped by their cluster's draft cell type.
    폐암 종양 미세환경(TME)의 세포 유형별 구성 막대그래프 — Lambrechts 2018(Nat Med) 카탈로그 대응.
    """
    by_type = summary.groupby("draft_celltype")["n_cells"].sum()
    # Order by the canonical lineage order (paper), with anything else (e.g. Unknown) last.
    order = [t for t in LUNG_TUMOR_MARKERS if t in by_type.index]
    ...
    labels = list(by_type.index)[::-1]        # top-to-bottom on the y-axis
    ...
    ax.set_title("NSCLC tumor microenvironment — cell-type composition\n"
                 "(draft labels; lineages per Lambrechts et al., Nat Med 2018)")
    ax.margins(x=0.18)
    fig.tight_layout()
    out = os.path.join(RESULTS_DIR, "nsclc_chart.png")
    fig.savefig(out, dpi=150, bbox_inches="tight")
    plt.close(fig)
    print(f"==> Saved NSCLC composition chart: {out}")
...
def main():
    ...
    # NSCLC cell-type composition chart (Lambrechts 2018-style TME catalog).
    plot_composition(summary)
    # Per-lineage cluster figures (Lambrechts 2018 Figs 2-5 style) + stromal distribution.
    for _cfg in LINEAGE_FIGS:
        plot_lineage(adata, celltype_df, _cfg)
    plot_stromal_distribution(summary)
