...
# =============================================================================
# run_gpu_reanalysis.sh  (scBaseCount HCC — independent GPU reanalysis wrapper)
# Runs 02_gpu_reanalysis.py on the GPU. Per BioIDE 헌장 제4조 this is GPU-first; it
# refuses to run without CUDA (no silent CPU fallback that would quietly take hours).
# Writes all outputs into $GHBIO_RESULTS.
#
# The pooled HCC atlas can be large; subsample for a fast draft run, e.g.:
#   bash run_gpu_reanalysis.sh --max-cells 60000
# =============================================================================
...
echo "==> [02] Independent GPU reanalysis (scBaseCount HCC) → ${RESULTS}"
