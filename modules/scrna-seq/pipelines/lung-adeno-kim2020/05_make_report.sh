...
# =============================================================================
# 05_make_report.sh  (Kim 2020 lung adeno — INDEPENDENT GPU reanalysis report)
...
SAMPLE="Kim et al., Nature Communications 2020 — 폐선암(lung adenocarcinoma) 전이 진행 단일세포 (208,506 cells · 44 patients · 58 samples · nLung/tLung/mLN/mBrain/PE)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_lung_kim2020_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building lung-adeno independent-reanalysis report PDF"
...
cells=meta.get("cells","208506"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
n_mal=meta.get("malignant_epithelial_cells","?"); n_norm=meta.get("normal_epithelial_cells","?")

logo = b64(os.environ["LOGO"])
cover = f"""<div class="cover">
  <img class="logo" src="data:image/svg+xml;base64,{logo}">
  <h1>GHBIO AI Co-Scientist</h1>
  <div class="sub">폐선암(lung adenocarcinoma) <b>독립 GPU 재현</b> 리포트 — Kim 2020</div>
  <div class="rule"></div>
  <div class="meta">
    데이터셋: {html.escape(os.environ['SAMPLE'])}<br>
    <b>{cells} cells</b> · 우리 세포유형 <b>{ntypes}</b> · Leiden 클러스터 <b>{nclust}</b> · 악성/정상 상피세포 <b>{n_mal}</b>/<b>{n_norm}</b><br>
    방법(헌장 제1조): 저자 라벨 미사용 · 저자 UMI 행렬 → GPU(PyTorch) 스케일·PCA → Harmony → Leiden → marker 주석 → 비지도 GMM 악성 분리 → 저자 라벨·전이 축 주장 대비 독립 검증
  </div>
  <div class="fields">
...
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Kim 2020)의 '
       '<b>저자 라벨</b>과 <b>정상→종양→전이 축 주장</b>과 대조했습니다. 아래는 두 분석이 '
       '얼마나 수렴하는지에 대한 정량 지표와 판정입니다.</p>']
...
for fn, cap in [("validation_bars.png","일치도 지표 (세포유형 ARI·NMI·악성 판정 F1)"),
                ("confusion_celltype.png","세포유형 혼동행렬 — 저자 라벨 대비 우리 유형"),
                ("progression_malignant.png","조직 기원별 악성 상피세포 비율 — 정상 대비 종양/전이 편중")]:
    if exists(fn):
...
FIGS = [
  ...
  ("umap_origin.png","조직 기원 UMAP (정상 폐→종양→전이)",
   "각 세포가 어느 조직(정상 폐·원발 종양·림프절·뇌 전이·흉수)에서 왔는지를 색으로 표시했습니다. 기원은 세포 라벨이 아니라 시료 정보입니다."),
  ("umap_malignant.png","독립 악성/정상 상피세포 판정 (비지도 GMM)",
   "상피세포를 폐선암 악성 서명 − 정상 폐포/기도 분화점수로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("composition.png","조직 기원별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, 각 조직 기원의 조성 비교(%). 전이 림프절(mLN)의 골수성 침윤 등을 볼 수 있습니다."),
]
...
if os.path.exists(os.path.join(R,"malignant_epithelial.csv")):
    de=list(csv.DictReader(open(os.path.join(R,"malignant_epithelial.csv"))))[:20]
    der="".join(f"<tr><td>{d.get('rank','')}</td><td class='mono'>{html.escape(d.get('gene',''))}</td>"
                f"<td>{d.get('log2fc','')}</td></tr>" for d in de)
    appendix+=("<h2>부록 B. 악성 vs 정상 상피세포 DE (우리 판정 기준)</h2>"
