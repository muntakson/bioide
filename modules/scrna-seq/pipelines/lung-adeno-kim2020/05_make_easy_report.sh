...
# =============================================================================
# 05_make_easy_report.sh  (폐선암 lung adeno scRNA-seq — 고등학생 버전 보고서)
# Turn the high-school-level report markdown (written by the AI panel's
# "🎓 고등학생 버전 보고서" button) into ONE branded GHBIO PDF.
#
# Self-contained: it needs ONLY the report markdown. The UMAP + progression
# figures are folded in when present, but nothing here depends on other pngs.
#
...
RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/lung-adeno-kim2020/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (폐선암 lung adeno)"
...
if [[ -f "${RESULTS}/progression_malignant.png" ]]; then
  {
    printf '\n\n## 조직별 암세포 비율\n\n'
    printf '![정상 폐→종양→림프절/뇌 전이로 갈수록 암세포 비율이 어떻게 달라지는지](%s)\n' "${RESULTS}/progression_malignant.png"
    printf '\n*그림: 정상 조직(회색)과 종양·전이 조직(빨강)에서 암세포(악성 상피세포) 비율이 어떻게 다른지 보여줍니다.*\n'
  } >> "$REPORT_MD"
fi
printf '\n\n<div class="foot">GHBIO Co-Scientist · 폐선암(lung adenocarcinoma) 전이 진행 단일세포 scRNA-seq (Kim 2020) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 폐선암 단일세포" \
