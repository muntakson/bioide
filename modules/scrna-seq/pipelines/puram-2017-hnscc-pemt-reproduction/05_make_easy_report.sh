...
# =============================================================================
# 05_make_easy_report.sh  (Puram 2017 HNSCC p-EMT 재현)
...
cat "$SRC" > "$REPORT_MD"
if [[ -f "${RESULTS}/umap.png" ]]; then
  {
    printf '\n\n## 세포 지도 (UMAP)\n\n'
    printf '![성향에 따라 세포를 배치한 지도 — 가까울수록 하는 일이 비슷합니다](%s)\n' "${RESULTS}/umap.png"
    printf '\n*그림: 점 하나가 세포 하나입니다.*\n'
  } >> "$REPORT_MD"
fi
if [[ -f "${RESULTS}/program_heatmap.png" ]]; then
  {
    printf '\n\n## 발현 프로그램 (p-EMT 포함)\n\n'
    printf '![악성세포에서 도출한 발현 프로그램들](%s)\n' "${RESULTS}/program_heatmap.png"
  } >> "$REPORT_MD"
fi
printf '\n\n<div class="foot">GHBIO Co-Scientist · Puram 2017 두경부암(HNSCC) p-EMT 재현 · 작성일 %s · 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"
