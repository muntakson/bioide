...
# =============================================================================
# 05_make_easy_report.sh  (췌장암 PDAC scRNA-seq — 고등학생 버전 보고서)
...
RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/pancreatic-cancer-sc-rna-seq/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (췌장암 PDAC)"
...
printf '\n\n<div class="foot">GHBIO Co-Scientist · 췌장암(PDAC) 단일세포 scRNA-seq (Peng 2019) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

# --- Render --------------------------------------------------------------------
pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 췌장암 단일세포" \
