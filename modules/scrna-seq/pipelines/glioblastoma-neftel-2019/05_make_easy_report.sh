...
# =============================================================================
# 05_make_easy_report.sh  (교모세포종 GBM scRNA-seq — 고등학생 버전 보고서)
# Turn the high-school-level report markdown (written by the AI panel's
# "🎓 고등학생 버전 보고서" button) into ONE branded GHBIO PDF.
#
# Self-contained: it needs ONLY the report markdown. The atlas UMAP figure is
# folded in when present, but nothing here depends on the other Scanpy pngs.
#
...
RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/glioblastoma-neftel-2019/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (교모세포종 GBM)"
...
printf '\n\n<div class="foot">GHBIO Co-Scientist · 갑상선암(PTC) 단일세포 scRNA-seq (Neftel 2019) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

# --- Render --------------------------------------------------------------------
pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 갑상선암 단일세포" \
