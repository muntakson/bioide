...
# =============================================================================
# 05_make_report.sh  (Pu 2021 PTC — INDEPENDENT GPU reanalysis report)
...
SAMPLE="Neftel et al., Cell 2019 — 교모세포종(GBM) Smart-seq2 단일세포 (7,930 cells · 28 tumors)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_gbm_neftel2019_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building GBM independent-reanalysis report PDF"
...
cells=meta.get("cells","158577"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
n_mal=meta.get("malignant_thyrocyte_cells","?"); n_norm=meta.get("normal_thyrocyte_cells","?")

logo = b64(os.environ["LOGO"])
cover = f"""<div class="cover">
  <img class="logo" src="data:image/svg+xml;base64,{logo}">
  <h1>GHBIO AI Co-Scientist</h1>
  <div class="sub">교모세포종(GBM) <b>독립 GPU 재현</b> 리포트 — Pu 2021</div>
  <div class="rule"></div>
  <div class="meta">
    데이터셋: {html.escape(os.environ['SAMPLE'])}<br>
    <b>{cells} cells</b> · 우리 세포유형 <b>{ntypes}</b> · Leiden 클러스터 <b>{nclust}</b> · 악성/정상 갑상선세포 <b>{n_mal}</b>/<b>{n_norm}</b><br>
    방법(헌장 제1조): 저자 라벨 미사용 · 저자 카운트 행렬 → GPU(PyTorch) 스케일·PCA → Harmony → Leiden → marker 주석 → 비지도 GMM 악성 분리 → 저자 주장 대비 독립 검증
  </div>
  <div class="fields">
...
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Pu 2021)의 '
       '<b>공개된 주장</b>(세포유형 구성·marker·악성 갑상선세포 특징)과 대조했습니다. 아래는 두 분석이 '
...
for fn, cap in [("validation_bars.png","일치도 지표 (marker 재현·악성 서명·조성)"),
                ("marker_recovery.png","저자 marker 재현 — 우리 클러스터에서의 발현")]:
    if exists(fn):
...
FIGS = [
  ("umap_celltypes.png","독립 재분석 — 세포유형 UMAP (marker 기반, 저자 라벨 미사용)",
   "저자 상태 라벨 없이 marker로 명명한 세포유형 UMAP입니다. 악성세포와 비악성(대식·T·희소돌기)세포를 구분합니다."),
  ("umap_state.png","독립 재분석 — 악성 4상태 (AC/MES/NPC/OPC-like)",
   "악성세포를 Neftel의 네 메타모듈 서명으로 점수화해 최고 상태로 배정한 UMAP입니다."),
  ("butterfly.png","Neftel 2D 상태 지도 (butterfly)",
   "세로축=분화(AC/MES)↔전구(NPC/OPC), 가로축=상단 NPC/OPC·하단 AC/MES. 네 극과 그 사이 hybrid 연속체를 보여줍니다."),
]
...
if os.path.exists(os.path.join(R,"malignant_thyrocyte.csv")):
    de=list(csv.DictReader(open(os.path.join(R,"malignant_thyrocyte.csv"))))[:20]
    der="".join(f"<tr><td>{d.get('rank','')}</td><td class='mono'>{html.escape(d.get('gene',''))}</td>"
                f"<td>{d.get('log2fc','')}</td></tr>" for d in de)
    appendix+=("<h2>부록 B. 악성 vs 정상 갑상선세포 DE (우리 판정 기준)</h2>"
