...
# =============================================================================
# 05_make_easy_report.sh  (간세포암 HCC / TLS scRNA-seq — 고등학생 버전 보고서)
# Turn the high-school-level report markdown (written by the AI panel's
# "🎓 고등학생 버전 보고서" button) into ONE branded GHBIO PDF.
#
# Self-contained: it needs ONLY the report markdown. The UMAP + TLS figures are
# folded in when present, but nothing here depends on other pngs.
#
# Reads GHBIO_RESULTS (the pipeline's own project results dir). Output:
#   <results>/GHBIO_고등학생_리포트.pdf   ← fixed filename the AI panel opens
#
# Requires: pandoc, wkhtmltopdf.
# =============================================================================

RESULTS="${GHBIO_RESULTS:-${HOME}/ghbio-workspace/projects/hcc-tls-lu2022/results}"
SRC="${RESULTS}/step4_ai_report_easy.md"
OUT="${RESULTS}/GHBIO_고등학생_리포트.pdf"
REPORT_DATE="$(date +%Y-%m-%d)"

echo "==> [05] 고등학생 버전 보고서 PDF 생성 (간세포암 HCC / TLS)"

for tool in pandoc wkhtmltopdf; do
  command -v "$tool" >/dev/null 2>&1 || { echo "ERROR: '$tool' 이(가) 없습니다." >&2; exit 1; }
done
[[ -f "$SRC" ]] || { echo "ERROR: ${SRC} 없음 — 먼저 AI 패널에서 '🎓 고등학생 버전 보고서' 버튼을 눌러 주세요." >&2; exit 1; }
...
cat "$SRC" > "$REPORT_MD"
if [[ -f "${RESULTS}/umap_celltypes.png" ]]; then
  {
    printf '\n\n## 분석 지도 (UMAP)\n\n'
    printf '![세포들을 성향에 따라 배치한 지도 — 비슷한 세포일수록 가까이 모입니다](%s)\n' "${RESULTS}/umap_celltypes.png"
    printf '\n*그림: 점 하나가 세포 하나이고, 가까이 모인 점일수록 하는 일(유전자 발현)이 비슷합니다. 색이 다른 무리는 서로 다른 종류의 세포입니다.*\n'
  } >> "$REPORT_MD"
fi
if [[ -f "${RESULTS}/tls_by_site.png" ]]; then
  {
    printf '\n\n## 종양 속 림프 기지(TLS)\n\n'
    printf '![정상 간→종양→전이로 갈수록 B세포·CXCL13⁺ Tfh와 CXCL13이 어떻게 달라지는지](%s)\n' "${RESULTS}/tls_by_site.png"
    printf '\n*그림: 종양 조직 안에 면역세포(B세포·여포도움 T세포)가 모여 "림프 기지(TLS)"를 만드는지, 그리고 이들을 불러모으는 신호물질 CXCL13이 어디서 높은지 보여줍니다.*\n'
  } >> "$REPORT_MD"
fi
if [[ -f "${RESULTS}/progression_malignant.png" ]]; then
  {
    printf '\n\n## 조직별 암세포 비율\n\n'
    printf '![정상 간→종양→전이로 갈수록 암세포 비율이 어떻게 달라지는지](%s)\n' "${RESULTS}/progression_malignant.png"
    printf '\n*그림: 정상 조직(회색)과 종양·전이 조직(빨강)에서 암세포(악성 간세포) 비율이 어떻게 다른지 보여줍니다.*\n'
  } >> "$REPORT_MD"
fi
printf '\n\n<div class="foot">GHBIO Co-Scientist · 간세포암(HCC) 다세포 생태계·종양 내 TLS 단일세포 scRNA-seq (Lu 2022) · 작성일 %s · 이 문서는 교육용 고등학생 버전 보고서이며 임상적 증거가 아닙니다.</div>\n' "$REPORT_DATE" >> "$REPORT_MD"

pandoc "$REPORT_MD" -f gfm -t html5 -s \
  --metadata title="GHBIO 고등학생 리포트 — 간암 속 림프 기지(TLS) 단일세포" \
