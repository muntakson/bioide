...
# =============================================================================
# 00_setup_env.sh  (Lu 2022 HCC ecosystem · TLS 3차 림프구조 — INDEPENDENT GPU reanalysis)
# BioIDE 헌장 제4조: 현대 GPU·Python 스택으로 재분석합니다. 이 파이프라인은
# 저자 라벨을 소비하지 않고(제1조) 새로 짠 코드로 세포유형·악성 간세포를 다시
# 도출하고, 종양 조직 안에 생기는 3차 림프 구조(TLS: B세포·형질세포·CXCL13⁺ Tfh·
# TLS 케모카인 CXCL13/CCL19/CCL21) 신호를 정량합니다. Scanpy + PyTorch(GPU) +
# Harmony + scikit-learn 이 필요합니다. 다른 예제와 같은 공유 venv(~/ghbio-venv)를
# 재사용합니다.
#
...
echo "==> [00] GPU reanalysis environment for Lu 2022 HCC / TLS: ${VENV}"
...
echo ""
echo "==> [00] Done. Next: 1. Download the Lu 2022 HCC matrix (GEO GSE149614)."
