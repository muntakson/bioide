...
# =============================================================================
# 05_make_report.sh  (Lu 2022 HCC / TLS — INDEPENDENT GPU reanalysis report)
...
#
# Options (all optional):  --results DIR / --author NAME / --out FILE
...
SAMPLE="Lu et al., Nature Communications 2022 — 간세포암(HCC) 다세포 생태계·종양 내 TLS 단일세포 (71,915 cells · 10 patients · 21 samples · Normal/Tumor/PVTT/Lymph)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_hcc_tls_lu2022_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building HCC/TLS independent-reanalysis report PDF"
...
cells=meta.get("cells","71915"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
n_mal=meta.get("malignant_hepatocyte_cells","?"); n_norm=meta.get("normal_hepatocyte_cells","?")

logo = b64(os.environ["LOGO"])
cover = f"""<div class="cover">
  <img class="logo" src="data:image/svg+xml;base64,{logo}">
  <h1>GHBIO AI Co-Scientist</h1>
  <div class="sub">간세포암(HCC) 생태계 · 종양 내 <b>TLS(3차 림프 구조)</b> 독립 GPU 재현 리포트 — Lu 2022</div>
  <div class="rule"></div>
  <div class="meta">
    데이터셋: {html.escape(os.environ['SAMPLE'])}<br>
    <b>{cells} cells</b> · 우리 세포유형 <b>{ntypes}</b> · Leiden 클러스터 <b>{nclust}</b> · 악성/정상 간세포 <b>{n_mal}</b>/<b>{n_norm}</b><br>
    방법(헌장 제1조): 저자 라벨 미사용 · 저자 UMI 행렬 → GPU(PyTorch) 스케일·PCA → Harmony → Leiden → marker 주석 → 비지도 GMM 악성 분리 → TLS 모듈(B/형질·CXCL13⁺ Tfh·TLS 케모카인) 점수화 → 저자 라벨·진행·TLS 주장 대비 독립 검증
  </div>
  <div class="fields">
...
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Lu 2022)의 '
       '<b>저자 라벨</b>·<b>정상→종양→전이 축 주장</b>·<b>종양 내 TLS 형성 주장</b>과 대조했습니다. 아래는 '
       '두 분석이 얼마나 수렴하는지에 대한 정량 지표와 판정입니다.</p>']
...
for fn, cap in [("validation_bars.png","일치도 지표 (세포유형 ARI·NMI·라벨 정확도)"),
                ("confusion_celltype.png","세포유형 혼동행렬 — 저자 라벨 대비 우리 유형"),
                ("progression_malignant.png","조직 부위별 악성 간세포 비율 — 정상 대비 종양/전이 편중"),
                ("tls_validation.png","종양 내 TLS 세포(B세포·CXCL13⁺ Tfh·CXCL13) 부위별 분포")]:
    if exists(fn):
...
FIGS = [
  ...
  ("umap_site.png","조직 부위 UMAP (정상 간→종양→전이)",
   "각 세포가 어느 조직(정상 간·원발 종양·문맥 종양전 PVTT·전이 림프절)에서 왔는지를 색으로 표시했습니다. 부위는 세포 라벨이 아니라 시료 정보입니다."),
  ("umap_malignant.png","독립 악성/정상 간세포 판정 (비지도 GMM)",
   "간세포를 HCC 악성 서명 − 정상 간대사 분화점수로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("umap_tls.png","TLS 니치 점수 UMAP (B/형질·CXCL13⁺ Tfh·TLS 케모카인)",
   "3차 림프 구조를 구성하는 프로그램(B·형질세포, CXCL13⁺ 여포도움 T, CXCL13/CCL19/CCL21 케모카인)이 어느 세포에서 켜지는지 점수로 표시했습니다."),
  ("composition.png","조직 부위별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, 각 조직 부위의 조성 비교(%). 종양 조직의 면역세포(B·T/NK) 침윤 등을 볼 수 있습니다."),
  ("tls_by_site.png","조직 부위별 TLS 세포(B세포·CXCL13⁺ Tfh)와 CXCL13",
   "종양 내에 림프 구조가 형성되는지 — B세포·CXCL13⁺ Tfh 비율과 평균 CXCL13 발현을 부위별로 비교합니다."),
]
...
if os.path.exists(os.path.join(R,"malignant_hepatocyte.csv")):
    de=list(csv.DictReader(open(os.path.join(R,"malignant_hepatocyte.csv"))))[:20]
    der="".join(f"<tr><td>{d.get('rank','')}</td><td class='mono'>{html.escape(d.get('gene',''))}</td>"
                f"<td>{d.get('log2fc','')}</td></tr>" for d in de)
    appendix+=("<h2>부록 B. 악성 vs 정상 간세포 DE (우리 판정 기준)</h2>"
