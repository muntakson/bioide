...
# -*- coding: utf-8 -*-
"""
02_gpu_reanalysis.py  (Kumar 2022, gastric cancer — INDEPENDENT GPU reanalysis)

BioIDE 헌장 제1조 (Reproduce, don't consume): we start from the authors' RAW
per-sample CSV count matrices (GSE183904, 48 samples; raw sequencing was withheld
for patient privacy, and NO cell-type labels are distributed) and RE-DERIVE the
whole analysis with our own, freshly-written, GPU-accelerated code:

  1. load every per-sample CSV count matrix (genes × cells) and merge on the
     COMMON gene set (feature lists can differ across samples),
  ...
  9. marker-based cell-type annotation (canonical gastric-ecosystem signatures),
 10. an INDEPENDENT malignant-vs-normal EPITHELIAL split (unsupervised GMM on a
     gastric-carcinoma malignancy − normal-gastric-differentiation signature) —
     the authors annotated malignant cells with their own pipeline; we deliberately
     use a different, unsupervised method so agreement in step 3 means two
     independent routes converge (제6조).
...
recovery, malignant gastric-differentiation-score drop, elevated malignant
proliferation, and malignant enrichment in tumour vs normal tissue when the
per-sample tissue label is derivable).
...
Outputs (into $GHBIO_RESULTS):
  ...
  epithelial_summary.csv       our epithelial split counts + mean scores (GDS, prolif)
  composition_by_site.csv      cell-type % per tissue site (if derivable)
  malignant_by_site.csv        malignant-epithelial % among epithelial per site
...
# ---- canonical gastric-ecosystem markers (for INDEPENDENT annotation) --------
# Markers named by Kumar et al. 2022 + standard canonical lineage markers.
CELL_TYPE_MARKERS: dict[str, list[str]] = {
    "Epithelial": ["EPCAM", "KRT8", "KRT18", "KRT19", "KRT7", "CDH1", "MUC1", "TFF1", "TFF3", "KRT20"],
...
# Gastric-carcinoma malignant epithelial signature (up) vs normal gastric
# differentiation signature (down) — Kumar et al. 2022 report malignant cells
# losing normal gastric-mucosa identity while gaining a carcinoma program.
MALIGNANT_SIG = ["CEACAM5", "CEACAM6", "KRT17", "CLDN3", "CLDN4", "CLDN7", "S100A4",
                 "S100P", "MMP7", "REG4", "OLFM4", "SPINK1", "MDK", "LGALS4"]
# Gastric Differentiation Score (GDS) genes — high in normal gastric mucosa
# (pit/mucous, chief, parietal, endocrine lineages), lost in malignant cells.
DIFFERENTIATION_SIG = ["TFF1", "TFF2", "MUC5AC", "MUC6", "GKN1", "GKN2", "GIF", "LIPF",
                       "PGA3", "PGA4", "PGA5", "PGC", "ATP4A", "ATP4B", "CHGA"]
# Cell-cycle / proliferation program (Kumar: malignant epithelium is proliferative).
PROLIF_SIG = ["MKI67", "TOP2A", "PCNA", "CCNB1", "CCNB2", "CENPF", "UBE2C", "BIRC5", "TYMS"]
EPITHELIAL_TYPES = {"Epithelial"}

# a broad known-gene set used only to orient each CSV (genes on rows or columns?)
_KNOWN_GENES = {g for genes in CELL_TYPE_MARKERS.values() for g in genes}
_KNOWN_GENES |= set(MALIGNANT_SIG) | set(DIFFERENTIATION_SIG) | set(PROLIF_SIG)
_KNOWN_GENES |= {"ACTB", "GAPDH", "B2M", "MALAT1", "PTPRC", "VIM", "FN1", "MT-CO1", "MT-ND1"}

# tissue-site canonicalisation + a normal→tumour severity rank (for enrichment claim)
SITE_RANK = {"Normal": 0, "Tumor": 1}


def map_site(token: str) -> str:
    """Best-effort tumour/normal call from a sample-name token. Kumar's CSV file
    names may not encode this (a separate mapping table holds it) — when nothing
    matches we default to 'Tumor' and the site-dependent claim (C4) degrades to N/A."""
    s = token.upper()
    if any(k in s for k in ("NORMAL", "ADJ", "NAG", "NONTUM", "NON-TUM", "_N", "CTRL")):
        return "Normal"
    if any(k in s for k in ("TUMOR", "TUMOUR", "_T", "CANCER", "CA", "PT", "MT", "LN")):
        return "Tumor"
    return "Tumor"
...
def read_csv_matrix(path: str) -> ad.AnnData | None:
    """Read one per-sample CSV count matrix into a cells×genes AnnData.

    GEO CSV deposits are usually genes×cells (gene symbols on the row index,
    cell barcodes on the header), but not always — orient by whichever axis
    overlaps the known-gene set more, so a transposed file still loads correctly."""
    try:
        with _open(path) as fh:
            df = pd.read_csv(fh, index_col=0)
    except Exception as e:  # unreadable / non-CSV member of the tar
        print(f"    WARNING: could not read {os.path.basename(path)} ({e}) — skipping.", file=sys.stderr)
        return None
    if df.shape[0] == 0 or df.shape[1] == 0:
        return None
    idx_hits = sum(1 for g in df.index.astype(str) if g in _KNOWN_GENES)
    col_hits = sum(1 for g in df.columns.astype(str) if g in _KNOWN_GENES)
    if col_hits > idx_hits:
        # genes are on the columns → cells are the index; already cells×genes
        genes = df.columns.astype(str)
        X = sparse.csr_matrix(df.to_numpy(dtype=np.float32))
        cells = df.index.astype(str)
    else:
        # genes are on the row index → transpose to cells×genes
...
def sample_meta(path: str) -> tuple[str, str]:
    """From GSM5573xxx_sample12.csv.gz → (sample='sample12', site='Tumor')."""
    base = os.path.basename(path)
    stem = re.sub(r"\.csv(\.gz)?$", "", base, flags=re.IGNORECASE)
    tokens = stem.split("_")
    if tokens and re.match(r"^GSM\d+$", tokens[0]):
        tokens = tokens[1:]
    sample = "_".join(tokens) if tokens else stem
    site = map_site(stem)
    return sample, site
...
def main() -> None:
    ...
    ap.add_argument("--source", default=os.path.expanduser(
        "~/ghbio-tutorial/data/gastric-kumar2022/unpacked"))
    ...
    matrices = sorted(glob.glob(os.path.join(args.source, "**", "*.csv*"), recursive=True))
    matrices = [m for m in matrices if re.search(r"\.csv(\.gz)?$", m, flags=re.IGNORECASE)]
    if not matrices:
        print(f"ERROR: no *.csv under {args.source} (run step 1 first).", file=sys.stderr)
        sys.exit(1)

    # --- load + merge every per-sample matrix on the COMMON gene set -----------
    with progress(f"Loading {len(matrices)} per-sample CSV matrices"):
        adatas = []
        for mp in matrices:
            a = read_csv_matrix(mp)
            if a is None or a.n_obs == 0:
                continue
            sample, site = sample_meta(mp)
            a.obs["sample"] = sample
            a.obs["patient"] = sample     # per-file mapping to patient isn't distributed; sample is the batch
            a.obs["site"] = site
    ...
    print(f"    merged: {adata.n_obs:,} cells × {adata.n_vars:,} common genes "
          f"from {adata.obs['sample'].nunique()} samples", flush=True)
    ...
    # The CSV→sparse→merge path can leave X with unsorted/duplicate indices or
    # float64; rank_genes_groups reads X column-wise and is sensitive to that,
    # which can crash a native routine (SIGSEGV) on real data. Force canonical
    ...
    sc.tl.score_genes(adata, sig_dif, score_name="gds_score", use_raw=False)   # gastric differentiation score
    if sig_pro:
    ...
    if n_epi > 20:
        ...
        score = (adata.obs.loc[epi_mask, "malignancy_score"]
                 - adata.obs.loc[epi_mask, "gds_score"]).to_numpy().reshape(-1, 1)
    ...
    comp_site = None
    has_two_sites = "site" in adata.obs and adata.obs["site"].nunique() > 1
    if has_two_sites:
    ...
    # epithelial split summary (GDS drop + proliferation, used by validation C2/C3)
    dsum = (adata.obs[adata.obs["malignant_call"] != "n/a"]
            ...
            .agg(n_cells=("malignancy_score", "size"),
                 ...
                 mean_gds=("gds_score", "mean"),
                 mean_prolif=("prolif_score", "mean")).round(4).reset_index())
    dsum.to_csv(os.path.join(R, "epithelial_summary.csv"), index=False)

    # malignant enrichment by tissue site (C4) — only when tissue is derivable
    if n_epi > 20 and has_two_sites:
        epi = adata[epi_mask].copy()
        by = pd.DataFrame({
            "site": epi.obs["site"].values,
            "is_mal": (epi.obs["malignant_call"] == "malignant epithelial").astype(int).values,
            "prolif": epi.obs["prolif_score"].values,
        })
        agg = by.groupby("site").agg(n_epithelial=("is_mal", "size"),
    ...
    prov = {
        ...
        "method": "per-sample CSV merge (common genes) + QC + normalize + HVG + GPU PyTorch (ScaleData + PCA-SVD) + Harmony + Leiden/UMAP + marker annotation + GMM malignant epithelial split",
