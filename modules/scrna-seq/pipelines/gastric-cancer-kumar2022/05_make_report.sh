...
# =============================================================================
# 05_make_report.sh  (Kumar 2022 gastric — INDEPENDENT GPU reanalysis report)
...
SAMPLE="Kumar et al., Cancer Discovery 2022 — 위암(gastric cancer) 단일세포 (>200,000 cells · 31 patients · 48 samples)"
OUT=""
while [[ $# -gt 0 ]]; do
  case "$1" in
...
OUT="${OUT:-${RESULTS}/GHBIO_gastric_kumar2022_report.pdf}"
PY="${HOME}/ghbio-venv/bin/python"
LOGO="$(cd "$(dirname "$0")" && pwd)/ghbio-logo.svg"

echo "==> [05] Building gastric independent-reanalysis report PDF"
...
cells=meta.get("cells","200000"); ntypes=meta.get("celltypes","?"); nclust=meta.get("clusters","?")
...
cover = f"""<div class="cover">
  ...
  <div class="sub">위암(gastric cancer) <b>독립 GPU 재현</b> 리포트 — Kumar 2022</div>
  <div class="rule"></div>
  <div class="meta">
...
val = ['<div class="pagebreak"></div><h1>독립 검증 결과 (헌장 제2조)</h1>',
       '<p>저자 라벨을 전혀 쓰지 않고 얻은 우리 독립 결과를, 여기서 처음으로 원 논문(Kumar 2022)의 '
       '<b>공개된 주장</b>(세포유형 구성·marker·악성 상피세포 특징)과 대조했습니다. 아래는 두 분석이 '
...
for fn, cap in [("validation_bars.png","일치도 지표 (marker 재현·악성 서명·증식)"),
...
FIGS = [
  ...
  ("umap_malignant.png","독립 악성/정상 상피세포 판정 (비지도 GMM)",
   "상피세포를 위암 악성 서명 − 위 분화점수로 비지도 GMM 분리한 결과. 저자 라벨을 참고하지 않았습니다."),
  ("composition.png","조직별 — 세포유형 조성 (독립 재분석)",
   "우리 세포유형 기준, 정상·종양 조직의 조성 비교(%). 조직 라벨을 도출할 수 있을 때만 표시됩니다."),
]
