...
# -*- coding: utf-8 -*-
"""
03_validate_vs_authors.py  (Kumar 2022, gastric — INDEPENDENT VALIDATION · 헌장 제2조)

Step 2 re-derived cell types and a malignant/normal epithelial split WITHOUT any
author labels — none are distributed on GEO for GSE183904. THIS step checks
whether our independent GPU reanalysis reaches the SAME conclusions as the paper.
Because there is no per-cell "answer key", we validate against the paper's
PUBLISHED CLAIMS and quantify agreement:

  C1. Lineage / marker recovery — did we recover the paper's major lineages
      (epithelial, T/NK, B, plasma, myeloid, endothelial, fibroblast), and are each
  ...
  C2. Malignant epithelial cells have a LOWER gastric-differentiation score (GDS:
      TFF1/MUC5AC/GKN1/LIPF/PGC…) than normal epithelial cells — malignant cells
      lose normal gastric-mucosa identity (Kumar et al. 2022).
  C3. Malignant epithelial cells are more PROLIFERATIVE (higher MKI67/TOP2A/…
      program) than normal epithelial cells.
  C4. Malignant epithelial cells are enriched in TUMOUR vs NORMAL tissue
      (only evaluable when a per-sample tissue label is derivable).

Inputs  (from $GHBIO_RESULTS, written by step 2):
  gpu_reanalysis.h5ad, celltype_composition.csv, epithelial_summary.csv,
  malignant_by_site.csv (optional)
...
EXPECTED_LINEAGES = ["Epithelial", "T-NK cell", "B cell", "Plasma cell",
...
# --- C2/C3. malignant epithelial: lower GDS + higher proliferation ----------
print("==> C2/C3: malignant GDS drop + proliferation")
gds_delta = None
prolif_delta = None
esum_path = os.path.join(R, "epithelial_summary.csv")
if os.path.exists(esum_path):
    es = pd.read_csv(esum_path).set_index("malignant_call")
    both = {"malignant epithelial", "normal epithelial"}.issubset(es.index)
    if both and "mean_gds" in es.columns:
        gds_delta = float(es.loc["normal epithelial", "mean_gds"] - es.loc["malignant epithelial", "mean_gds"])
        gds_ok = "AGREE" if gds_delta > 0.05 else ("PARTIAL" if gds_delta > 0 else "DISAGREE")
        rows.append(("C2 악성 분화점수 감소 (normal − malignant GDS)",
                     "정상 대비 악성 상피세포의 위 분화점수 하락폭 (>0 이면 주장 방향 일치)",
                     round(gds_delta, 4), gds_ok))
    else:
        rows.append(("C2 악성 분화점수 감소", "epithelial_summary.csv 에 두 그룹이 없어 평가 불가", "n/a", "N/A"))
    if both and "mean_prolif" in es.columns:
        prolif_delta = float(es.loc["malignant epithelial", "mean_prolif"] - es.loc["normal epithelial", "mean_prolif"])
        prolif_ok = "AGREE" if prolif_delta > 0.05 else ("PARTIAL" if prolif_delta > 0 else "DISAGREE")
        rows.append(("C3 악성 증식 상승 (malignant − normal proliferation)",
                     "정상 대비 악성 상피세포의 증식 점수 상승폭 (>0 이면 주장 방향 일치)",
                     round(prolif_delta, 4), prolif_ok))
    else:
        rows.append(("C3 악성 증식 상승", "epithelial_summary.csv 에 증식 점수가 없어 평가 불가", "n/a", "N/A"))
else:
    rows.append(("C2 악성 분화점수 감소", "epithelial_summary.csv 없음", "n/a", "N/A"))
    rows.append(("C3 악성 증식 상승", "epithelial_summary.csv 없음", "n/a", "N/A"))

# --- C4. malignant epithelial enrichment in tumour vs normal tissue ---------
print("==> C4: malignant enrichment by tissue")
enrich_delta = None
enrich_ok = "N/A"
mbs_path = os.path.join(R, "malignant_by_site.csv")
if os.path.exists(mbs_path):
    mbs = pd.read_csv(mbs_path, index_col=0)
    if {"Normal", "Tumor"}.issubset(mbs.index) and "pct_malignant" in mbs.columns:
        norm = float(mbs.loc["Normal", "pct_malignant"])
        tum = float(mbs.loc["Tumor", "pct_malignant"])
        enrich_delta = tum - norm
        enrich_ok = "AGREE" if enrich_delta > 15 else ("PARTIAL" if enrich_delta > 0 else "DISAGREE")
        rows.append(("C4 악성 조직 편중 (tumor − normal 악성%)",
                     f"normal {norm:.1f}% → tumor {tum:.1f}% (양수면 주장 일치)",
                     round(enrich_delta, 2), enrich_ok))
    else:
        rows.append(("C4 악성 조직 편중", "정상/종양 두 조직 라벨을 파일명에서 도출할 수 없어 평가 불가", "n/a", "N/A"))
else:
    rows.append(("C4 악성 조직 편중", "malignant_by_site.csv 없음 (조직 라벨 미도출)", "n/a", "N/A"))
...
if gds_delta is not None:
    barvals.append(("C2 GDS↓", min(max(gds_delta * 4, 0), 1)))       # scale delta into 0–1 for display
if prolif_delta is not None:
    barvals.append(("C3 prolif↑", min(max(prolif_delta * 4, 0), 1)))
if enrich_delta is not None:
    barvals.append(("C4 enrich", min(max(enrich_delta / 50, 0), 1)))
...
if verds and n_dis == 0 and n_agree >= max(2, len(verds) - 1):
    vlabel = "재현됨 (AGREE)"
    vtext = "우리 독립 GPU 재분석이 원 논문(Kumar 2022)의 핵심 주장(세포계통·악성 상피세포 특징·증식)을 재현합니다."
elif n_dis >= 2:
    vlabel = "불일치 (DISAGREE)"
    vtext = "독립 재분석이 원 논문 주장과 여러 지점에서 어긋납니다 — 파라미터/주석 재검토가 필요합니다."
else:
    vlabel = "부분 일치 (PARTIAL)"
    vtext = "핵심 주장은 대체로 재현되나 일부 주장(증식/편중)에서 차이가 있습니다."

lines = [
    "BioIDE 독립 검증 결과 (헌장 제2조) — Kumar 2022 위암 (저자 라벨 미배포 → 논문 주장과 대조)",
    "=" * 64,
    f"판정(Verdict): {vlabel}",
    vtext,
    "",
...
if gds_delta is not None:
    lines.append(f"C2 악성 분화점수 하락폭(정상−악성 GDS): {gds_delta:.3f}")
if prolif_delta is not None:
    lines.append(f"C3 악성 증식 상승폭(악성−정상): {prolif_delta:.3f}")
c4 = summary[summary["claim"].str.startswith("C4")]
if len(c4) and c4["verdict"].iloc[0] not in ("N/A",):
    lines.append(f"C4 악성 조직 편중(tumor−normal %): {c4['value'].iloc[0]} → {c4['verdict'].iloc[0]}")
lines += [
    "",
    "주의(제6조): 저자 라벨이 없어 '정답 대조'가 아니라 논문이 보고한 주장을 우리 독립 결과가",
    "정성·정량적으로 재현하는지 확인한 것입니다. 저자와는 다른 비지도 GMM으로 악성 상피세포를",
    "가렸으므로, 일치는 두 독립 경로의 수렴을 뜻합니다(정답 보증이 아닙니다).",
]
